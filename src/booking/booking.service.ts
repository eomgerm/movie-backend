...
export class BookingService {
  ...
  async createBooking(scheduleId: string, userId: string, seats: string[]) {
    ...
    try {
      ...
      const isOccupied = await queryRunner.manager.findOne(BookingSeats, {
        relations: { booking: true },
        where: { booking: { scheduleId }, seat: In(seats) },
      });

      if (isOccupied) {
        return new BadRequestException(`Seats ${isOccupied.seat} Occupied`);
      }
      ...
      return new InternalServerErrorException(`DB ERROR: ${error.message}`);
    } finally {
